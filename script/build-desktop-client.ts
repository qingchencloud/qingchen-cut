...
function run(
	...
	const runEnv = {
		...env,
		[pathKey]: [dirname(process.execPath), env[pathKey] ?? ""]
			.filter(Boolean)
			.join(delimiter),
	};
...
function which(name: string): string {
	const result = spawnSync(
		"powershell",
		["-NoProfile", "-Command", `(Get-Command ${name}).Source`],
		{
			cwd: root,
			encoding: "utf8",
			windowsHide: true,
	...
	if (!p || !existsSync(p))
		throw new Error(`找不到 ${name}，请先安装或放到 PATH`);
	return p;
}
...
run(process.execPath, ["run", "build:web"], "Next standalone build", {
	...process.env,
	NODE_ENV: "production",
	QC_LOCAL_CLIENT: "1",
...
run(
	...
	[
		...
		"--outfile",
		exePath,
		join(root, "packages", "cli", "src", "qingchen-cut-studio.ts"),
	],
	"Bun compile",
);
...
if (!existsSync(packagedServerJs))
	throw new Error(`缺少 Next standalone 服务入口: ${packagedServerJs}`);
...
if (existsSync(join(whisperDir, "whisper-cli.exe"))) {
	copyRequired(
		join(whisperDir, "whisper-cli.exe"),
...
2. 程序会在本机启动原 Web 编辑器，并自动打开独立客户端窗口
...
注意：
- 当前版本是本地客户端包，界面使用原 Web 编辑器并通过独立客户端窗口显示；Windows 下优先使用 Edge app window，找不到 Edge 时才回退到默认浏览器。
- 所有素材路径和输出都在本机，不会上传。
- AI 自动剪辑主路径是仓库内 MCP/CLI/headless core；请参考 docs/ai-agent-quickstart.md 和 docs/qingchen-cut-ai-skill.md。
`;
	...
	JSON.stringify(
		{
			...manifest,
			outDir,
			files: readdirSync(outDir).map((file) => basename(file)),
		},
		null,
		2,
	),
